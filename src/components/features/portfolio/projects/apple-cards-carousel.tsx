  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="group relative z-10 flex h-80 w-56 flex-col items-start justify-start overflow-hidden rounded-3xl bg-gray-100 shadow-lg transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] dark:bg-neutral-900"
      >
        {/* Enhanced gradient overlay */}
        <div className="absolute inset-0 z-30 bg-gradient-to-b from-black/60 via-black/20 to-black/80 transition-opacity duration-300 group-hover:from-black/40 group-hover:via-black/10 group-hover:to-black/60" />
        ...
        <div className="relative z-40 p-6 h-full flex flex-col justify-between">
          ...
          <div>
            <motion.h3
              layoutId={layout ? `title-${card.title}` : undefined}
              className="text-left font-sans text-xl font-bold text-white leading-tight [text-wrap:balance] drop-shadow-lg md:text-2xl"
            >
              {card.title}
            </motion.h3>
            
            {/* Subtle accent line */}
            <div className="mt-3 h-1 w-12 bg-gradient-to-r from-blue-400 to-purple-500 rounded-full opacity-80" />
          </div>
        </div>
        ...
        <div className="absolute inset-0 z-20 rounded-3xl border border-white/10" />
      </motion.button>
    </>
  );
};
